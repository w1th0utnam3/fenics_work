...
def compile_cffi(module_name: str,
    ...
    ffi.cdef(code_h)
    lib = ffi.compile(tmpdir=path, verbose=verbose)

    return lib


def import_cffi(module_name: str, path: str = None):
    """Imports a module generated by CFFI."""

    if path is not None:
        mod = importlib.import_module("{}.{}".format(path,module_name))
    else:
...
def run_benchmark(test_case: TestCase, test_fun_names: Dict, codes: List[Tuple[str, str]],
    ...
    path = "bench-{:%Y%m%d-%H%M%S-%f}".format(datetime.now())
    module_cache = dict()
    module_counter = 0
    ...
    def compile_module(index: int, args, macros=None):
        ...
        if index in module_cache.keys():
            ...
            compile_cffi("_benchmark_{}".format(module_counter), codes[index][0], codes[index][1],
                         ...
                         path=path,
                         verbose=verbose)
            ffi, lib = import_cffi("_benchmark_{}".format(module_counter), path=path)
